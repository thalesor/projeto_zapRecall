...
const Card = ({total, setFace, face, index, status, cardData, onStatusChange}) =>
{
    return(
    <>
    <div class={`card ${face === 'back' ? 'flipped' : ''} ${status}`}>
        <div className='card-inner'>
            <div class='card-front'>
                <div class="card-header">
                    <span class="card-indicator">{index}/{total}</span>
                </div>
                <div class="card-content">
                    <h2 class="card-question">{cardData.question}</h2>
                </div>
                <div class="card-footer">
                    <a onClick={()=> setFace('back')} class="card-switch"><img src="assets/turn.png" alt="" /></a>
                </div>
            </div>
